...
//        Toast.makeText(getApplicationContext(), "NFC TAG", Toast.LENGTH_SHORT).show();
    ...
    public static final String CHARS = "0123456789ABCDEF"; //16진수
...
//                editor.putString("NFC_ID", "" + Alarm_Fragment_2.getTime() + " : " + Alarm_Fragment_3.getMinute());
                ...
                if (dbHelper.isExistData(TAG_ID)) { // update
                    dbHelper.updataNFCData(TAG_ID, writeText.getText().toString());
                    Log.d("UPDATE", TAG_ID +writeText.getText().toString());
                } else { //add
                    dbHelper.saveNFCData(new NFCData(TAG_ID,writeText.getText().toString()));
                    Log.d("INSERT", TAG_ID +writeText.getText().toString());

                }
                Log.d("NFCData", TAG_ID +writeText.getText().toString());
                finish();
            } else {
